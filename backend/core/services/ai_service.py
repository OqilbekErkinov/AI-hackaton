...
def generate_quiz(topic):
    """
    Berilgan mavzu bo'yicha 5 ta savoldan iborat JSON formatdagi test yaratadi.
    """
    prompt = f"""
Siz talabalar uchun qiziqarli va foydali test (viktorina) yaratuvchi AI ustasiz.
Berilgan mavzu: "{topic}"

Shu mavzuga doir 5 ta test savolini tuzing. Har bir savol uchun 4 ta variant (A, B, C, D) bo'lsin va faqat 1 ta to'g'ri javob bo'lsin.
JAVOB FAQAT QAT'IY JSON FORMATIDA BO'LISHI SHART! Hech qanday qo'shimcha matn (```json yoki shunga o'xshash) qo'shmang.
...
"""
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Siz faqat JSON qaytaruvchi botsiz."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7
        )
